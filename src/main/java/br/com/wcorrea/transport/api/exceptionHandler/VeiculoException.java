...
public class VeiculoException extends DefaultExceptionHandler {
    ...
    @ExceptionHandler({ControleKmPeriodoFuturoNaoPermitido.class})
    public ResponseEntity<Object> handleControleKmPeriodoFuturoNaoPermitido(ControleKmPeriodoFuturoNaoPermitido ex, WebRequest request, Locale loc) {
        String userMessage = messageSource.getMessage("recurso.controle_km_periodo_futuro_nao_permitido", null, loc);
        String developerMessage = ex.toString();
        List<ApiError> errors = Arrays.asList(new ApiError(userMessage, developerMessage, HttpStatus.NOT_ACCEPTABLE));
        return handleExceptionInternal(ex, errors, new HttpHeaders(), HttpStatus.NOT_ACCEPTABLE, request);
    }
